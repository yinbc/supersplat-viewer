 ...
 * Smooth Locomotion: Use left thumbstick (or remote touchpad) for movement and right thumbstick for snap turning
 *
...
class XrNavigation extends Script {
    ...
    cameraEntity: import('playcanvas').Entity | null = null;
    ...
    initialize() {
        ...
        if (!this.cameraEntity) {
            ...
            this.cameraEntity = foundByName as import('playcanvas').Entity | null;

            // If not found, search children for entity with camera component
            if (!this.cameraEntity) {
                for (const child of this.entity.children) {
                    const childEntity = child as import('playcanvas').Entity;
    ...
    findPlaneIntersection(origin: Vec3, direction: Vec3) {
    ...
    handleSmoothLocomotion(dt: number) {
        ...
        for (const inputSource of this.inputSources) {
            ...
            if (inputSource.handedness === 'left') {
                const { x, y } = this.getMoveAxes(inputSource.gamepad.axes);
    ...
    handleSnapTurning(inputSource: XrInputSource) {
